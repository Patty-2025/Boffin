    ...
    <div className="mx-auto w-full max-w-7xl animate-in fade-in duration-500 font-['Open_Sans',sans-serif]">
      <section className="overflow-hidden border border-slate-200 bg-white shadow-sm">
        ...
        {!loading && !error && activeOrder && <div className="grid gap-0 lg:grid-cols-[320px_1fr]">
          <div className="border-b border-slate-200 bg-slate-50 p-3 lg:border-b-0 lg:border-r lg:p-4"><p className="mb-2 text-xs font-bold uppercase tracking-wide text-slate-500">Your orders ({filteredOrders.length})</p><div className="space-y-1">{filteredOrders.map((order) => <button key={order.id} type="button" onClick={() => setSelectedOrderId(order.id)} className={`w-full border px-2.5 py-2 text-left transition ${order.id === activeOrder.id ? 'border-[#0080d1] bg-white shadow-sm' : 'border-slate-200 bg-slate-50 hover:bg-white'}`}><div className="flex items-center gap-2"><span className="shrink-0 font-mono text-[10px] font-bold text-slate-900">#{order.id.slice(-8).toUpperCase()}</span><p className="min-w-0 flex-1 truncate text-xs font-semibold text-slate-700">{getOrderTitle(order)}</p><StatusBadge status={(order.status || 'draft') as OrderStatus} /></div><div className="mt-1 flex items-center gap-2 text-[10px] text-slate-500"><span className="shrink-0 truncate">Due: {order.deadlineDate || order.deadline || 'Not set'}</span><span className="ml-auto shrink-0">{typeof order.totalCost === 'number' ? `$${order.totalCost.toFixed(2)}` : 'Price pending'}</span></div><div className="mt-1 h-0.5 bg-slate-200"><div className="h-full bg-[#13bdb0]" style={{ width: `${getProgress(order.status)}%` }} /></div></button>)}</div></div>
